...
/// Populate fts_symbols and fts_body inside a single transaction.
/// Clears both tables atomically before rebuilding; rolls back on any error.
...
pub fn search_bm25(conn: &Connection, query: &str, top_n: usize) -> Result<Vec<(i64, f32)>> {
    ...
    let rows: Vec<(i64, f32)> = stmt
        .query_map(params![sanitized, top_n as i64], |row| {
